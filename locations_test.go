...
func TestCountriesValues(t *testing.T) {
	q := make(url.Values)
	countres := Countries{IDs: []int64{1, 2, 3, 4, 5}}
	equals(t, url.Values{"countries_id": []string{"1,2,3,4,5"}}, countres.Values(q))
}

func TestProvidersValues(t *testing.T) {
	q := make(url.Values)
	providers := Providers{IDs: []int64{1, 2, 3, 4, 5}}
	equals(t, url.Values{"providers_id": []string{"1,2,3,4,5"}}, providers.Values(q))
}
